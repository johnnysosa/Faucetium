...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or FAUCETIUM.conf setting gen)\n"
...
// NOTE: Unlike wallet RPC (which use FAUCET values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "FAUCETIUM is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "FAUCETIUM is downloading blocks...");

...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(BitcoinUnits::FAUCET)
    {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance, std::vector<COutput> termDepositInfo)
{
    ...
    // Fetch sort flag
    bool sort_flag = GetArg("-sortFAUCETings", true);
    LogPrintf("sort FAUCETings flag = %b\n", sort_flag);
    ...
    ui->FAUCETTable->setRowCount(termDepositInfo.size());

    // actually update labels
    int nDisplayUnit = BitcoinUnits::FAUCET;
    // Disable sorting outside the for loop 
    ui->FAUCETTable->setSortingEnabled(false);
    ...
    for(int i=0;i<termDepositInfo.size();i++){
        ...
        if(curHeight>=releaseBlock){
            ui->FAUCETTable->setItem(i, 0, new QTableWidgetItem(QString("Matured (Warning: this amount is no longer earning interest of any kind)")));
            totalMatured += matureValue;
        }else{
            ui->FAUCETTable->setItem(i, 0, new QTableWidgetItem(QString("FAUCETed")));
            totalAccrued += (withInterest-termDeposit.nValue);
	    totalLocked  += termDeposit.nValue;
        }

        ui->FAUCETTable->setItem(i, 1, new QTableWidgetItem(BitcoinUnits::format(nDisplayUnit, termDeposit.nValue)));
        ui->FAUCETTable->setItem(i, 2, new QTableWidgetItem(BitcoinUnits::format(nDisplayUnit, withInterest-termDeposit.nValue)));
        ui->FAUCETTable->setItem(i, 3, new QTableWidgetItem(BitcoinUnits::format(nDisplayUnit, withInterest)));
        ui->FAUCETTable->setItem(i, 4, new QTableWidgetItem(BitcoinUnits::format(nDisplayUnit, matureValue)));
        ui->FAUCETTable->setItem(i, 5, new QTableWidgetItem(QString::number((term)/561)));

        if(!sort_flag){
            ui->FAUCETTable->setItem(i, 6, new QTableWidgetItem(QString::number(lockHeight)));
            ui->FAUCETTable->setItem(i, 7, new QTableWidgetItem(QString::number(releaseBlock)));
        }else{
            ui->FAUCETTable->setItem(i, 6, new QTableWidgetItem(QString::number(lockHeight).rightJustified(7,'0')));
            ui->FAUCETTable->setItem(i, 7, new QTableWidgetItem(QString::number(releaseBlock).rightJustified(7,'0')));
        }
        ...
        ui->FAUCETTable->setItem(i, 8, new QTableWidgetItem(QString(buffer)));
    }
    ...
    if(sort_flag){
     ui->FAUCETTable->setSortingEnabled(true);
    }
}
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("FAUCET")
    updateDisplayUnit();
}

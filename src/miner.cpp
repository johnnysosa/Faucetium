...
CBlockTemplate* CreateNewBlockWithAddress(std::string address)
{
    ...
    if(chainActive.Height()+1>=MINERFAUCETINGHEIGHT && chainActive.Height()+1<THEUNFORKENING){
        scriptPubKey = GetTimeLockScriptForDestination(CBitcoinAddress(address).Get(),chainActive.Height()+1+MINERFAUCETINGPERIOD);
    }
    else{
...
CBlockTemplate* CreateNewBlockWithKey(CReserveKey& reservekey)
{
    ...
    if(chainActive.Height()+1>=MINERFAUCETINGHEIGHT && chainActive.Height()+1<THEUNFORKENING){
        ...
        scriptPubKey = GetTimeLockScriptForDestination(dest,chainActive.Height()+1+MINERFAUCETINGPERIOD);
    }else{
...
static bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("FAUCETIUMMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock, true, NULL))
        return error("FAUCETIUMMiner: ProcessNewBlock, block not accepted");
...
void static BitcoinMiner(CWallet *pwallet, uint32_t minerI, uint32_t minerN, int nThreads)
{
	fGenerate = true;
	minerStopFlag = 0;
    LogPrintf("FAUCETIUMMiner started\n");
    srand(clock());
        ...
        while (true) {
            ...
            if(ma!=""){
            	if(validateAddress(ma)) {
                    LogPrintf("FAUCETIUMMiner: Mining to User supplied address %s\n",ma);
                    pblocktemplate= auto_ptr<CBlockTemplate>(CreateNewBlockWithAddress(ma));
                }else{
                    LogPrintf("FAUCETIUMMiner: WARNING! User supplied address is invalid, defaulting to keypool address.\n");
                    pblocktemplate= auto_ptr<CBlockTemplate>(CreateNewBlockWithKey(reservekey));
                }
            }else{
                LogPrintf("FAUCETIUMMiner: Mining to keypool address.\n");
                pblocktemplate= auto_ptr<CBlockTemplate>(CreateNewBlockWithKey(reservekey));
            }
            if (!pblocktemplate.get())
            {
                LogPrintf("FAUCETIUMMiner: ERROR! Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running FAUCETIUMMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("FAUCETIUMMiner terminated\n");
    ...
    {
        LogPrintf("FAUCETIUMMiner runtime error: %s\n", e.what());
        aligned_free(scratchpad);
        fGenerate = false;
        return;
    }

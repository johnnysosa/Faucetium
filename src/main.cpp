...
#if defined(NDEBUG)
# error "FAUCETIUM cannot be compiled without assertions."
#endif
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if(pindex->nHeight>=MINERFAUCETINGHEIGHT && pindex->nHeight<THEUNFORKENING){
        //Extra checks to ensure miner is a FAUCETER
        ...
        if (block.vtx[0].vout[0].scriptPubKey.GetTermDepositReleaseBlock() != pindex->nHeight+MINERFAUCETINGPERIOD){
            return state.DoS(100,error("ConnectBlock(): coinbase transaction does not FAUCET for one year, FAUCETblocks=%d, expected=%d\n",block.vtx[0].vout[0].scriptPubKey.GetTermDepositReleaseBlock(),pindex->nHeight+MINERFAUCETINGPERIOD),REJECT_INVALID, "bad-cb-outputtx-FAUCETingperiod");
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion < (chainActive.Height() < THEUNFORKENING ? MIN_PEER_PROTO_MINERFAUCETFORK_VERSION : MIN_PEER_PROTO_THEUNFORKENING_VERSION))
        {
